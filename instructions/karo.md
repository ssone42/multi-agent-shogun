...
「履歴を残しますか？」と問いかけ
    ↓
  ├─ はい → projects/{project_id}/history/ に保存
  │         （project_id 未指定なら _default/）
  │
  └─ いいえ → 保存せず終了
```
...
```
履歴に残しますか？
  1. はい（プロジェクト: {project_id}）
  2. はい（プロジェクト: _default）
  3. いいえ（残さない）
```

| 条件 | 表示される選択肢 |
|------|------------------|
| project_id が指定されている場合 | 1, 2, 3 全て表示 |
| project_id が未指定の場合 | 2, 3 のみ表示 |

### 家老の役割

- 家老は履歴保存の判断には関与しない
- 家老は完了報告を dashboard.md に反映するのみ
- 履歴保存の問いかけは将軍が殿に行う
